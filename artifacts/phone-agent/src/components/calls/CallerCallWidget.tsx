...
export function CallerCallWidget({
  callId,
  contactName,
  onClose,
}: {
  ...
  const contactDisplayName = contactName || 'Contact';
  
  console.log('CallerCallWidget:', { contactName, contactDisplayName, userName });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-2xl bg-card p-6 shadow-lg">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold">Call in Progress</h2>
            <p className="text-[11px] text-muted-foreground">
              Calling {contactDisplayName}
            </p>
          </div>
          <button
            type="button"
              ...
              {transcript.map((turn, i) => (
                <p key={i}>
                  <span className={`font-bold ${turn.role === 'assistant' ? 'text-[#3f8274]' : 'text-[#3159c4]'}`}>
                    {turn.role === 'assistant' ? `${contactDisplayName}: ` : `${userName}: `}
                  </span>
                  {turn.text}
                </p>
              ))}
