...
export const createHistory = async (
  conversationId: string,
  data: Omit<History, 'id' | 'createdAt' | 'updatedAt' | 'conversationId'>,
): Promise<History> => {
  const response = await apiFetch(`${API_BASE_URL}/conversations/${conversationId}/history`, {
    method: 'POST',
...
export const createHistoryWithConversation = async (
  data: Omit<History, 'id' | 'createdAt' | 'updatedAt' | 'conversationId'>,
): Promise<History> => {
