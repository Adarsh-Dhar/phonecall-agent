...
/**
 * Service-account voice call — handles real calls between personal users
...
export type ServiceVoiceCallStatus = 'idle' | 'connecting' | 'active' | 'error';
...
/**
 * Manages the WebSocket + mic-capture lifecycle for a service-account voice call.
 * `ServiceCallWidget` consumes this hook and stays a thin UI layer.
 */
export function useServiceVoiceCall(callId: string) {
  const [status, setStatus] = useState<ServiceVoiceCallStatus>('idle');
